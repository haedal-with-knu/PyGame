...
class Snake:
    def __init__(self,pos): #생성자로 객체를 초기화
        self.bodies =[pos]

    def move(self, key): # 뱀을 이동하는 메서드
        ...
        if key == K_LEFT:
            ...
            ypos += 1
        head = (xpos, ypos) #head의 좌표를 초기화한다
...
def add_food(snake): #임의의 장소에 먹이를 배치
    while True:
...
def paint(snake, message):#화면 전체 그리기
    SURFACE.fill((0, 0, 0))
    snake.draw()
